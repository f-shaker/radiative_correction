...
def create_submission_script(file_idx):
    submission_file_skdetsim = "submit_skdetsim_no_g_fin" + str(file_idx) + ".sh"
    out_skdetsim = "./skdetsim_output/skdetsim_out_no_g_fin" + str(file_idx) + ".zbs"
    input_pg_file = "../pg_files/pg_mu_ID_no_g_fin" + str(file_idx) + ".txt"
...
for i in range(file_start, file_stop+1, file_step):
    dump = "skdetsim_dump_no_g_fin" + str(i) + ".txt"
